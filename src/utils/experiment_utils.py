...
def init_runs(num_runs: int,
    ...
    for i, (m_conf, o_conf) in enumerate(zip(search(model_configs), search(optim_configs))):
        model_fn_partial = partial(model_fn,
                                   **m_conf,
...
def run_valid(run: Run, evaluate_fn: Callable, tr_set, vl_set, 
              save_path: Path, num_trials: int = 1, 
              higher_is_better: bool = True) -> float:
    """Run validation for a single run configuration."""
    from rich.pretty import pprint as log
    
    set_seed(run.seed)
    is_better = lambda a, b: a > b if higher_is_better else a < b
    log(run.config)
...
def _run_seq(exp, tr_set, vl_set, save_path):
    ...
    for run in exp.runs:
        score = run_valid(run, exp.evaluate_fn, tr_set, vl_set, 
                         save_path / exp.name, exp.num_valid_trials, 
...
def _run_par(exp, tr_set, vl_set, save_path):
    ...
    for run in exp.runs:
        id_ = exp._fire.remote(run, tr_set, vl_set, save_path / exp.name, 
                              exp.num_valid_trials, exp.higher_is_better)
        remotes[id_] = run
...
def run_test(run: Run, evaluate_fn: Callable, tr_set, vl_set, ts_set,
             save_path: Path, num_trials: int = 5, higher_is_better: bool = True):
