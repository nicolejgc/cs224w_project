...
_LARGE = {
    "model": dict(
        num_hidden=partial(integer, a=16, b=512), alpha=partial(uniform, a=0, b=0)
    ),
    "optim": dict(
...
_SMALL_FF = {
    "model": dict(
        num_hidden=partial(integer, a=64, b=72),  # 114-138
        alpha=partial(uniform, a=0, b=1),
    ),
    "optim": dict(
        lr=partial(uniform, 1e-3, b=1e-2), weight_decay=partial(uniform, a=1e-3, b=4e-3)
    ),
}

_ONE_FF = {
    "model": dict(
        num_hidden=lambda: 68,  # 114-138
...
_ONE_FF_MC = {
    "model": dict(num_hidden=lambda: 65, alpha=partial(uniform, a=0, b=0)),
    "optim": dict(
        lr=lambda: 0.009868199084919982, weight_decay=lambda: 0.0017345516681916279
    ),
}
